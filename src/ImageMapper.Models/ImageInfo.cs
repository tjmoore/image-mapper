    ...
    double Longitude) : BasicFileInfo(BasicFileInfo)
{
    ...
    [JsonConstructor]
    public ImageInfo(string id, string fileName, string filePath, double latitude, double longitude, string url = "")
        : this(new BasicFileInfo(id, fileName, filePath, url), latitude, longitude)
    {
    }
    ...
    public ImageInfo(ImageInfo imageInfo, string url)
        : this(new BasicFileInfo(imageInfo.Id, imageInfo.FileName, imageInfo.FilePath, url), imageInfo.Latitude, imageInfo.Longitude)
    {
    }
}
