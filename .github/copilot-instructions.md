...
- Runtime flow:
  ...
  2. ImageWorkerService scans configured ImageFolders, uses MetadataExtractor to pull EXIF/geotag info and builds ImageInfo objects.
  3. Web (Blazor + Leaflet) fetches image metadata from Services and renders markers on the map.
- Configuration:
  - ImageFolders (array) lives in appsettings.json for ImageMapper.Web.
...
- Aspire-first workflow:
  ...
  - Use `aspire run` when iterating on multi-component behavior — it reliably composes Web + diagnostics.
- Configuration keys:
...
- Do not directly generate Dockerfiles or docker-compose.yml, use aspire publish to generate
